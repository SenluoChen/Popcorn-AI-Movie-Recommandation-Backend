    ...
    new BatchGetCommand({
      RequestItems: {
        [tableName]: {
          ...
          // Project only what we need for ranking + UI.
          // This prevents DynamoDB from returning large plot fields, which can add seconds.
          ProjectionExpression: '#imdbId,#title,#year,#tmdbId,#poster_path,#productionCountry,#language,#genre,#tags,#keywords,#director,#moodTags,#vector',
          ExpressionAttributeNames: {
            '#imdbId': 'imdbId',
            '#title': 'title',
            '#year': 'year',
...
async function faissSearch({ baseUrl, vector, topK }) {
  ...
  const timeoutMs = Number(process.env.FAISS_TIMEOUT_MS || 2500);
